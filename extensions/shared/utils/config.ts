...
type ConfigValue = JsonValue[] | { [key: string]: JsonValue };
type PiSettingsValue = { [key: string]: JsonValue };

export function loadConfig(cwd: string, fileName: string): ConfigValue | undefined {
	return loadMergedJson(getConfigPaths(cwd, fileName));
}

export function loadPiSettings(cwd: string): PiSettingsValue | undefined {
	const config = loadConfig(cwd, "settings.json");
	return isPlainObject(config) ? config : undefined;
}
